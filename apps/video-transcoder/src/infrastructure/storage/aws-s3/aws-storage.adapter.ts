...
export class AwsS3StorageAdapter implements OnModuleInit, TranscoderStoragePort {
  private readonly RAW_VIDEOS_DIR = 'raw';
  private readonly ROOT_VIDEOS_DIR = 'videos';
  private readonly TRANSCODED_VIDEOS_DIR = 'transcoded';
  ...
  public getTranscodedFileKey(videoId: string) {
    return `${this.ROOT_VIDEOS_DIR}/${this.TRANSCODED_VIDEOS_DIR}/${videoId}.m3u8`;
  }

  public getRawFileKey(videoId: string) {
    return `${this.RAW_VIDEOS_DIR}/${this.RAW_VIDEOS_DIR}/${videoId}`;
  }

  public async getRawVideoFileAsReadableStream(videoId: string): Promise<Readable> {
    const key = this.getRawFileKey(videoId);

    this.logger.alert(`Downloading ${key} file as stream...`);
  ...
  public async uploadTranscodedVideoFileAsStream(
    ...
    const key = `${this.getTranscodedFileKey(videoId)}/${fileName}`;
  ...
  public async uploadTranscodedVideoDirectory(options: {
    ...
    return {
      hlsManifestKey: `${this.getTranscodedFileKey(videoId)}/${manifestFile}`,
    };
  }
}
