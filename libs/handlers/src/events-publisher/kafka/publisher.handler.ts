...
import type { KafkaPublisherOperationOptions } from '../types';

export interface KafkaEventPublisherResilienceConfig {
  maxRetries?: number;
  circuitBreakerThreshold?: number;
  halfOpenAfterMs?: number;
}

export interface KafkaEventPublisherHandlerConfig {
  host: string;
  port: number;
  service: string;
  logErrors?: boolean;
  ...
  resilienceOptions?: KafkaEventPublisherResilienceConfig;
}

export const KAFKA_EVENT_PUBLISHER_HANDLER_CONFIG = Symbol('KAFKA_EVENT_PUBLISHER_CONFIG');

@Injectable()
export class KafkaEventPublisherHandler implements OnModuleInit {
  ...
  public constructor(
    ...
    @Inject(KAFKA_EVENT_PUBLISHER_HANDLER_CONFIG)
    private readonly config: KafkaEventPublisherHandlerConfig,
  ) {
  ...
  private async publishToDlq(options: {
    ...
    this.logger.alert(`message for dlq`, dlqMessage);
  ...
  public async execute<TExecutionResult>(
    ...
    options: KafkaPublisherOperationOptions,
